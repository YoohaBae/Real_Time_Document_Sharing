...
async function duplicateCredentials(name, email) {
...
async function sendVerificationEmail(email) {
    ...
    try {
        ...
        let html_content = `http://iwomm.cse356.compas.cs.stonybrook.edu/verify?email=${encodeURIComponent(email)}&key=${key}`;
...
router.post('/signup', async (req, res) => {
    ...
    if (await duplicateCredentials(name, email)) {
        status = "ERROR";
        statusCode = 400;
    } else {
